...
import numpy as np
from curation_SC import fix_from_dentate2crblctx, mapping_parallel, compute_parallel, merge_SC_DTIapproach
...
#plot_your_matrix(SC, labels, 5, title='SC', cbar_name='weights', cmap_type='jet', figdim=(10, 10))


# # NESTED PROCEDURE
# # output of one function is the input for the following one

SC = merge_SC_DTIapproach(SCcur=SC_cur, SCstd=SC_std, idxdent=[103, 104, 105, 113, 114, 115])
...
SC_dL = fix_from_dentate2crblctx(SC, [103, 104, 105])
#From Right to Left
RL = 10
SC_d = fix_from_dentate2crblctx(SC_dL, [103 + RL, 104+RL, 105+RL])

# # Output = mapping of where parallel exists, other values = 0
SC_map = mapping_parallel(SC_d)
...
plot_2_matrices(matrix=norm_SC_std, labels=labels, step_labels=3,
                title='Whole brain SC - standard', cbar_name='weigths', cmap='jet',
                matrix1=norm_SC_std_crbl, labels1=labels[93:], step_labels1=1,
